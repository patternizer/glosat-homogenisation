...
for cycle in range(ncycle):
  ...
  if cycle>10:
    for s in range(nstn):
      diff = dnorm[:,s]-fill[:,s]
      flags[:,s] = changemissing( diff, nsig=2, nbuf=60, stderror=errorfn )
  ...
  for s in range(nstn):
    for f in numpy.unique(flags[:,s]):
      for m in range(12):
        mmask = numpy.logical_and( mnths==m, flags[:,s]==f )
        norms[mmask,s] = numpy.nanmean( data[mmask,s] - fill[mmask,s] )
